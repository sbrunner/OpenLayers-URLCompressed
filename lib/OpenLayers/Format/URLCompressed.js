...
OpenLayers.Format.URLCompressed = OpenLayers.Class(OpenLayers.Format, {
        ...
        if (data.charAt(0) == 'F') {
            ...
            while (data.length > 0) {
                if (data.charAt(0) == 'A') {
                    ...
                    results.push(this.parseFeature(data.substring(0, index)));
    ...
    parseFeature: function(data) {
        ...
        var index = data.search(/~/);
        if (index >= 0) {
            var attributesstring = data.substring(index + 1);
            data = data.substring(0, index + 1);

            var indexs = attributesstring.search(/~/);
            if (indexs >= 0) {
                var styledata = attributesstring.substring(indexs + 1).split("'");
                attributesstring = attributesstring.substring(0, indexs);
                for (var i = 0, len = styledata.length; i < len; ++i) {
                    var kv = decodeURIComponent(styledata[i]).split('*');
                    style[kv[0]] = kv[1];
                }
            }

            var attributesdata = attributesstring.split("'");
    ...
    extract: {
        ...
        'feature': function(feature) {
            ...
            if (feature.style && !isEmpty(feature.style)) {
                return geom.substring(0, geom.length - 1) + '~' + this.extractAttributes(feature.attributes) + '~' + this.extractAttributes(feature.style) + ")";
            }
